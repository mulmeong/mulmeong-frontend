...
import MapDataCredit from '@/features/mypage/components/MapDataCredit'
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-4 lg:flex-row">
        <div className="border-border-default relative flex flex-1 flex-col items-center rounded-sm border p-4">
          {/*
          자리를 두 줄로 미리 잡아 둔다. 고르기 전 문구가 두 줄이고 고른 뒤는 한
          줄이라, 높이를 안 잡으면 누르는 순간 아래 지도가 한 줄만큼 위로 튄다.
        */}
          {nation.error ? (
            // 지도는 그대로 두고 사유만 얹는다. 색이 왜 다 옅은지 알 수 있어야 한다.
            <p role="alert" className="text-danger min-h-9 text-center text-[12px]">
              {nation.error} · 방문 기록 없이 지도만 보여드려요
            </p>
          ) : (
            <p className="text-text-secondary min-h-9 text-center text-[12px]">
              {selected
