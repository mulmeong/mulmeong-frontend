  ...
  async function toggle() {
    ...
    try {
      ...
      if (cause instanceof ApiError && cause.status === 401) setLoginOpen(true)
        ...
        setError(
          cause instanceof Error ? cause.message : '찜을 변경하지 못했어요. 다시 시도해주세요.',
        )
    }
  }
  ...
  return (
    <>
      <button
        ...
        className={cn(
          'text-text-primary inline-flex size-8 shrink-0 items-center justify-center gap-2 rounded-sm hover:bg-surface-dim focus-visible:outline focus-visible:outline-2 disabled:opacity-50',
          className,
        )}
      >
        <svg
