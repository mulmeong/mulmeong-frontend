...
**이 가드를 제거하면 안 된다.** 조건 없는 지도 이동에는 별도로 캐시한 전국 마커를 사용한다. 미리보기 카드 목록을 지도 이동 때마다 다시 받지 않는다.

### 3-4. 전국 지도와 카드 미리보기 데이터 분리

- `useOnsens(REGION_PREVIEW_COUNT)`는 첫 화면 카드용으로 `/onsens?limit=6`을 조회한다. 지역·검색어가 있는 결과는 기존 전체 응답을 사용한다
- `useOnsenMapPoints`는 `/map/onsens`(API-301)의 지도 전용 요약 응답을 별도로 받는다. 첫 화면과 조건 없는 지도 이동에는 이 전국 데이터가 유지되어, 카드 6개 제한 때문에 마커가 누락되지 않는다
- API-301 요청은 `swLat=33&swLng=124&neLat=39&neLng=132&zoom=7`이다. 로컬 백엔드 `MapController`·`PlaceService`·`MapOnsensResponse` 기준이며, `zoom >= 8`에서 서버 집계가 반환되므로 기존 카카오 클러스터러에 전달할 개별 마커를 요청한다. 카메라의 실제 줌을 강제로 바꾸는 값이 아니다
- 지도 응답은 `id·name·lat·lng`으로 변환해 보관한다. 서버 요약 DTO에는 사진 URL·수질 등 추가 필드도 있으며, 실제 전송 필드를 네 가지로 줄이는 것은 서버의 별도 변경이 필요하다
- 전국 데이터는 **5분 캐시 + 진행 중 요청 공유**를 사용한다. 실패는 캐시하지 않고 지도 위에 재시도를 제공한다. 서버가 집계 또는 불완전한 마커 목록을 반환하면 성공한 전체 지도처럼 표시하지 않는다
- 지역·검색 조건이 있으면 그 결과가 지도 마커가 된다. 조건을 해제하면 전국 마커와 카드 미리보기를 각각 복원한다
- 선택은 `selectedId`로 유지하며, 미리보기 목록에 없으면 지도 데이터에서 장소를 찾는다. 상세는 선택 시 `/onsens/{id}`로 조회하며 **5분 캐시 + 진행 중 요청 공유**를 사용한다. 목록 정보가 없는 동안에는 이름과 로딩·오류·재시도를 표시한다

지도 전용 API는 로컬 백엔드에서 DB를 조회한다. **505개 레코드와 외부 API 505회 호출은 다르다.** `limit`만으로 외부 쿼터 절감이나 0.5초 응답을 보장하지 않으며, 배포된 서버의 계약·응답 크기·지연은 별도로 확인해야 한다.

### 3-5. 검색 후 지도 이동 규칙 (`focus`)
...
│    └── 장소 카드 최대 6개 (가로)
│         └── 지역을 고른 경우에만 전체보기 → 6개씩 페이지 탐색
├── 지금 이런 곳은 어때요 (전체 지역 미리보기일 때)
└── SidebarMagazine (지역 미리보기일 때)
```

`SidebarMagazine`은 지역 기사 1편을 사진·제목·읽기 링크로 보여준다. 데스크톱에서는 매거진 높이를 확보하고 위쪽 검색·장소 영역만 스크롤해 기사가 패널 아래에 잘리지 않게 한다. 모바일의 45dvh 패널에서는 검색부터 매거진까지 하나의 흐름으로 스크롤한다. 지역 장소의 전체 목록을 볼 때는 매거진을 감춰 목록 탐색에 집중한다.

### 5-2. 지역 칩은 필터, 하단은 장소 탐색

`REGIONS = ['서울','경기','인천','강원','충청','경상','전라','제주']`

기능명세서 MY-02는 **시·도 17단위**라고 되어 있지만, 지도 필터 1단계는 8개 광역 묶음이다. 포도알 지도(MY-02)와 **단위가 다르다** — 통합하려 하지 말 것. `REGION_VIEWS` 좌표는 명세에 없는 값이라 **대략치로 잡아둔 것**이고, 기획·BE에서 확정되면 교체한다.

⚠️ **권역 판정은 반드시 `regionOf()`를 쓴다** ([types/onsen.ts](../src/types/onsen.ts)). `address.startsWith('경상')` 같은 문자열 비교는 **틀린다** — 실제 데이터의 주소는 `'경북 울진군…'`·`'부산 동래구…'`처럼 시·도 약칭으로 시작해서 권역명과 겹치지 않는다. 실제로 이 방식 때문에 **524곳 중 440곳(충청·경상·전라 전부)이 필터에서 빠지고 있었다.**

`regionOf()`는 정식 `sido`(`'경상북도'`)를 먼저 보고, 없으면 주소 앞 토큰(`'경북'`)으로 넘어간다. 실데이터 524곳 중 `sido`는 결측 0건, `address`는 18건이 비어 있어 **`sido`가 우선이다.** 둘 다 모르면 `undefined`를 돌려준다 — 엉뚱한 권역에 섞이면 찾기 어려우므로 넘겨짚지 않는다.

권역을 고르면 별도의 시군구 칩이나 `'{권역} 전체'`를 반복하지 않고 [RegionPlaces.tsx](../src/features/map/components/RegionPlaces.tsx)의 장소 카드가 바로 나온다.

- 선택 상태는 기존 검정 지역 칩의 `aria-pressed`로 표현한다. 같은 칩을 다시 눌러도 선택을 유지하고, `'전체'`로 지역 조건을 해제한다
- 하단 제목은 `'{권역}에서 둘러보기'` 정도로 작게 표시한다. 지역 선택 전에는 `'먼저 둘러보기'`와 `'지역을 선택하면 더 많은 장소를 볼 수 있어요.'`를 표시한다. 선정 근거가 없는 항목을 추천으로 부르지 않는다
- 초기 탐색에서는 응답 순서대로 **최대 6개**만 가로로 보여준다. 카드 폭은 다음 카드 일부가 보이도록 제한하며, 실제로 넘칠 때만 이동 화살표를 표시한다
- 스와이프·트랙패드·이동 버튼·좌우 방향키로 탐색한다. 지역을 바꾸면 카드 스크롤과 전체 목록 페이지를 초기화한다
- **전체보기는 지역을 고른 경우에만 제공한다.** 첫 화면에서는 전국 총개수·총페이지·전체보기를 표시하지 않는다. 지역 전체보기는 같은 패널 안의 확장 목록으로 전환하며, 기존 `SearchResultItem`을 재사용해 **페이지당 6개**씩 교체한다. 미리보기로 돌아올 수 있다
- 전체보기와 페이지 이동 시 목록 제목으로 키보드 포커스를 옮긴다. 장소 선택은 기존 지도·상세 패널과 같은 `onSelect`를 쓴다
- 로딩 중에는 실제 카드와 같은 사진 비율·텍스트 행 높이의 스켈레톤을 표시한다. 카드 이동 버튼 자리도 유지해 데이터 도착 전후 매거진 위치가 흔들리지 않게 한다. 검색·전체 목록과 매거진에도 각 형태에 맞는 스켈레톤을 사용하며, 모션 줄이기 설정에서는 애니메이션을 끈다
- 오류(재시도)·장소 없음 상태를 제공하고, 사진이 없거나 로드에 실패한 카드는 온천 기호로 대체한다

**첫 화면 카드 요청은 `limit=6`으로 제한한다.** `features/map/constants.ts`의 `REGION_PREVIEW_COUNT`를 미리보기 렌더링·`useOnsens` 초기 조회·`handleSearch`의 조건 없는 분기가 공유한다. **마운트·'전체' 복귀·초기화·재시도 모두 같은 상한을 적용한다.** 지역·검색어가 있는 경우는 기존 전체 목록 계약을 유지하며, 서버 페이지네이션 도입 시 전체 접근과 한 번의 다운로드를 분리해야 한다.

`useOnsens`의 캐시 키에 `limit`이 들어간다. 부분 응답을 전체 조회의 캐시로 사용하지 않는다. 같은 조건의 진행 중 요청도 공유해 StrictMode 재실행과 빠른 필터 전환의 중복 요청을 막는다.

> ⚠️ `limit`은 **BE와 아직 합의되지 않은 파라미터다.** 목(`mockSearchOnsens`)은 `slice`로 흉내내고 있고, 실서버가 무시하면 505곳이 그대로 온다. 연동 시 `/onsens`의 `limit` 지원 여부를 먼저 확인할 것.

- 지역 목록 페이지 분리는 현재 클라이언트에서 처리한다. 배열 길이를 총개수로 사용할 수 있는 전제는 서버가 해당 조건의 완전한 목록을 반환한다는 계약이다. 향후 부분 응답에서는 `items`·다음 페이지 정보·선택적인 `totalCount`를 구분해야 한다

입력 중인 검색어는 제출하기 전까지 지역 탐색에 적용하지 않는다. 제출하면 기존 검색 결과 모드로 전환한다.
...
사이드바는 시안(1:479 / 1:555)의 좌표를 그대로 따르지 않는다. **팀 논의로 밀도·위계를 우선**하기로 해서 divider를 걷어내고 여백으로 섹션을 나눴다. 읽히는 순서는 `검색 → 지역 필터 → 장소 카드 → 추천`. `SearchResultItem`도 같은 이유로 구분선 대신 hover 배경을 쓴다.
...
- '지금 이런 곳은 어때요'의 전체보기는 아직 동작 없음. 지역 장소 전체보기는 페이지 탐색으로, 매거진 전체보기는 `/magazine`으로 연결됨
...
`MapPage`는 `selectedId: number`만 들고, 객체는 목록 또는 지도 데이터에서 되찾는다. **온천 객체 사본을 따로 들지 말 것** — 데이터가 재조회되면 사본이 낡는다.
...
첫 화면에서는 목록에 없는 장소를 전국 마커에서도 찾는다. 조건 변경 시 선택을 해제하고, 선택한 마커의 상세는 별도로 조회한다 (§3-4).
...
비동기 훅은 요청 번호 또는 effect 정리 시 비활성화하는 플래그로 늦게 도착한 이전 요청이 최신 결과를 덮어쓰지 않게 한다. 목록 훅의 요청 번호 패턴은 다음과 같다.
...
| `fetchOnsenMapPoints` | `GET /map/onsens?swLat=33&swLng=124&neLat=39&neLng=132&zoom=7`                       | API-301 요약 응답 → `OnsenMapPoint[]` |
