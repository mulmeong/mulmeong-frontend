...
`SidebarMagazine`은 사이드바 스크롤 영역 안에 있으며, 내용이 짧으면 `mt-auto`로 하단에 붙는다. 지역 장소의 전체 목록을 볼 때는 추천 콘텐츠를 감춰 목록 탐색에 집중한다.
...
- 로딩·오류(재시도)·장소 없음 상태를 제공하고, 사진이 없거나 로드에 실패한 카드는 온천 기호로 대체한다
